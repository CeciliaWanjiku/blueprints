(ns blueprints.graphql.resolvers.payment-source
  (:refer-clojure :exclude [type name])
  (:require [blueprints.graphql.authorization :as authorization]
            [blueprints.graphql.resolvers.util :refer [error-message]]
            [blueprints.graphql.resolvers.util.autopay :as autopay-util]
    ...
    (if (some? subs)
      ...
      (try
        (let [source  (tsource/by-id teller id)
              ...
              plan    (tplan/create! teller (autopay-util/plan-name teller license) :payment.type/rent rate)]
          (tsubscription/subscribe! customer plan {:source   source
                                                   :start-at (autopay-util/autopay-start customer)})
