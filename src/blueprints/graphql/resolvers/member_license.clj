(ns blueprints.graphql.resolvers.member-license
...
(defn- reassign-autopay!
  ...
  (try
    (let [license-after (d/entity (d/db conn) license)
          ...
          new-plan      (tplan/create! teller (autopay-util/plan-name teller license-after) :payment.type/rent rate)]
      (tsubscription/cancel! old-sub)
      (tplan/deactivate! old-plan)
      (tsubscription/subscribe! customer new-plan {:source   source
                                                   :start-on (autopay-util/autopay-start customer)})
